...
from .models import DownloadSettings, validate_settings
...
class MainWindow(QMainWindow):
    ...
    def _build_setup_panel(self) -> QWidget:
        ...
        url_actions.addWidget(self._button("Paste", "content_paste", self._paste_urls, compact=True))
        url_actions.addWidget(self._button("Import file", "file_open", self._import_urls, compact=True))
        url_actions.addStretch()
        url_actions.addWidget(self._button("Clear", "delete_sweep", self.urls_input.clear, compact=True))
    ...
    def _start_download(self) -> None:
        ...
        self._stopped_by_user = False
        self._update_mode = False
        self.batch_counts.setText(f"{len(urls)} URLs  •  0 files")
        command = build_command(settings, urls)
        self._append_log(f"$ {shlex.join(command)}\n", "info")
        self.status_label.setText(f"Downloading {len(urls)} URLs…")
    ...
    def _handle_output(self, text: str) -> None:
        ...
        for line in lines:
            ...
            if "[error]" in lowered or "error:" in lowered:
                ...
                self.skipped += 1
            elif not line.startswith("[") and os.path.isabs(line.strip().lstrip("✔ ")):
                self.downloaded += 1
                kind = "success"
            self._append_log(line, kind)
        self.batch_counts.setText(f"{len(self._collect_urls())} URLs  •  {self.downloaded} files")
    ...
    def _on_finished(self, exit_code: int) -> None:
        if self._line_buffer:
            self._append_log(self._line_buffer + "\n")
            self._line_buffer = ""
        ...
        if self._stopped_by_user:
            ...
            message = f"Download complete. {self.downloaded} files downloaded."
            kind = "success"
        else:
            message = f"Download failed with exit code {exit_code}."
            kind = "error"
